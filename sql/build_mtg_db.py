...
for scode in sets:
	...
	cursor.close()
	cnx.commit()
	cards = sorted(data[scode]['cards'], key=lambda x: x['id'])
	for c in cards:
		counters['cards']['counter'] += 1
		c_id = c.get('id', '***BAD DATA***')
		...
		c_image = c.get('imageName', '')

		try:
			cursor = cnx.cursor()
			sql = 'INSERT INTO cards (id, cid, layout, name, manacost, cmc, type, rarity, ctext, ftext, artist, num, p, t, l, mid, imagename, imgpath, setcode) VALUES (null,"' + c_id + '","' + c_layout + '","' + c_name + '","' + c_manaCost + '","' + c_cmc + '","' + c_type + '","' + c_rarity + '","' + c_text + '","' + c_flavor + '","' + c_artist + '","' + c_number + '","' + c_power + '","' + c_tough + '","' + c_loyal + '","' + c_multiId + '","' + c_image + '",null,"' + scode + '")'
			cardsql = sql;
			#print('CARD: ' + sql)
			cursor.execute(sql)
